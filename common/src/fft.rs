...
struct RaderDomain<F: Field, D: FftDomain<F>> {
    ...
    // prime root modulo p
    g: usize,
    // domain of size p-1, or of size >= 2p-3
    d: D,
...
impl<F: Field, D: FftDomain<F>> RaderDomain<F, D> {
    ...
    fn new_with_padding(w: F, p: usize, g: usize, d: D, d_inv: D) -> Self {
        assert!(d.n() >= 2 * p - 3);
        Self::new(w, p, g, d, d_inv)
    }
    ...
    fn new(w: F, p: usize, g: usize, d: D, d_inv: D) -> Self {
        ...
        b.resize(d.n(), F::zero()); // noop if w/o padding
        let b = d.fft(&b);
...
impl<F: Field, D: FftDomain<F>> FftDomain<F> for RaderDomain<F, D> {
    fn fft(&self, coeffs: &[F]) -> Vec<F> {
        ...
        // if padding
        if self.d.n() > n {
            a.resize(self.d.n() - (n - 1), F::zero());
            a.extend_from_within(1..n);
        }
        assert_eq!(a.len(), self.b.len());
...
mod tests {
    ...
    // p - domain size, must be prime
    // g = primitive_root(p)
    // conv_domain_size = p - 1 (w/o padding), or conv_domain_size >= 2p - 3 (with padding)
    fn _test_rader(p: usize, g: usize, conv_domain_size: usize) {
        use ark_bls12_381::Fq;
        let rng = &mut test_rng();

        let conv_domain_gen = gen::<Fq>(conv_domain_size);
        let conv_domain = NaiveDomain::new(conv_domain_gen, conv_domain_size);
        let conv_domain_inv = NaiveDomain::new(conv_domain_gen.inverse().unwrap(), conv_domain_size);
    ...
    // p - domain size
    // g = primitive_root(p)
    fn _test_rader_wo_padding(p: usize, g: usize) {
        _test_rader(p, g, p - 1);
    }

    #[test]
    fn test_rader_wo_padding() {
        _test_rader_wo_padding(3, 2);
        _test_rader_wo_padding(23, 5);
        _test_rader_wo_padding(47, 5);
    }

    // p - domain size
    // g = primitive_root(p)
    // conv_domain_size >= 2p - 3
    fn _test_rader_with_padding(p: usize, g: usize, conv_domain_size: usize) {
        _test_rader(p, g, conv_domain_size);
    }

    #[test]
    fn test_rader_with_padding() {
